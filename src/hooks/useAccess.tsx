...
import { isAdmin as rolesAreAdmin, isUnconfigured, tabsFor, unmappedRoles } from "@/lib/access";
...
async function readLocal(userId: string): Promise<{ roles: string[]; grants: string[] }> {
...
async function load(userId: string, force: boolean): Promise<void> {
  ...
  try {
    ...
    if (force || roles.length === 0) {
      const { error: fnErr } = await supabase.functions.invoke("sync-my-role");
      if (fnErr) {
  ...
  loadedFor = userId;
  setState({ loading: false, roles, grants, loaded: true, error });
}
...
export function useAccess() {
  ...
  useEffect(() => {
    ...
    if (!userId) {
      ...
      loadedFor = null;
      inFlight = null;
      setState({ loading: false, roles: [], grants: [], loaded: false, error: null });
      return;
    }
  ...
  return {
    ...
    tabs: tabsFor(roles, preview ? [] : local.grants),
    isAdmin: admin,
