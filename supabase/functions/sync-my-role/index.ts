...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!lookupUrl || !lookupKey) {
      // Configuration error, not the caller's fault. Say so plainly so the
      // failure is diagnosable instead of looking like "you have no roles".
      console.error("sync-my-role: ARCHITECT_ROLE_API_URL or _KEY is not set");
      return json({ error: "Role lookup is not configured" }, 503);
    }
