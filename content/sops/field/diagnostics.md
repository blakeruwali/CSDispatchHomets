...
**A diagnosis is a cause you have measured, not a part you suspect.** "Bad capacitor" is not a diagnosis. "Run capacitor reads 21.4 µF against 45 µF rated, compressor drawing locked-rotor amps on start attempt" is a diagnosis. The difference is not paperwork. The first one is a guess that happens to be right most of the time; the second one is the reason we can charge for expertise, stand behind the repair, and win the argument when a customer or a manufacturer pushes back six months later.
...
**It is the single biggest thing that keeps you off unpaid return trips.** Almost every callback traces to the same root: a part was replaced, the thing that killed it was not, and it killed the new one too. You went back for free. A contactor replaced without checking why it welded is a job you are going to do twice.

**It is what makes a big ticket stick.** A customer approves a $1,400 repair when the tech shows them a reading and explains what it means. They approve a $260 repair and argue about it when the tech just says the part is bad. The measurement is not bureaucracy — it is the sales tool.

**It protects you when it goes wrong.** If a compressor fails a month later and your file shows correct subcooling, correct amp draw, and a clean airflow reading at the time you left, that is a system that failed. Without them, it is a tech who missed something. Same job, two completely different conversations.
...
- **Has anyone else worked on it?** Including the homeowner. This is the highest-yield question on the list and the one most often skipped.
...
|---|---|
| Smell of gas / positive leak detection | Shut off gas at the appliance. Get people out if it is strong. Utility gets called, then dispatch. Do not create an ignition source, and that includes light switches. |
...
| Suspected cracked heat exchanger | **Shut down and red-tag. Do not restart it to "show" the customer.** Second set of eyes required before condemning — see below. |
...
Nothing in this document — not the time it takes, not the fee conversation, not a customer who is in a hurry — outranks this table.

### 3. Verify the complaint yourself {#verify}

Run the system and observe the failure with your own eyes and instruments. If the customer says it is not cooling, put a thermometer on it. If they say it is noisy, hear the noise.

**If you cannot reproduce it, say so and go to `#no-fault` below.** Do not "fix" a complaint you never witnessed. That is how a working capacitor gets replaced and the actual intermittent fault comes back next week with your name on the last invoice.
...
The two most-replaced parts in this trade are capacitors and contactors, and they are the two most often replaced without measurement because they are cheap and usually the answer. **Usually is not a diagnosis.** Measure them anyway — it takes forty seconds and it is the difference between a professional finding and a lucky guess.

**Never condemn a part you have not tested.** If you cannot test it on site with what you carry, say so in the note and explain what you are inferring from.

### 7. Ask what killed it {#root-cause}

The failed part is usually a symptom of something upstream. You do not have to solve it to a certainty every time, but you do have to look — and write down what you found or ruled out.
...
**"Replaced the part, didn't check why" is the single most common cause of a callback**, and a callback comes out of a paid slot you could have billed. This is not the office asking for extra work — it is the step that keeps you from doing this job twice.
...
- **A system that should be replaced rather than repaired** → the repair still gets priced, so the customer can see the comparison they are actually being asked to make. Then the replacement conversation.
- **A part we have to order** → priced, with the return visit scheduled before you leave.
- **No fault found** → still a written outcome and still a record. See below.
...
1. **Take the full reading set anyway.** A healthy baseline is genuinely valuable — it is what the next visit compares against.
...
5. **The diagnostic is still charged.** Service was rendered. You spent the time, you took the readings, and you left them with a documented healthy baseline and a plan. Say so with a straight back.
...
4. **Check the manufacturer warranty status** from the serial you captured. Condemning a compressor that is still covered, and not saying so, is the kind of mistake a customer never forgives.

The second-opinion call is not doubt about your ability. It is standard practice on the three or four decisions per year that are worth the most money and carry the most risk, and it puts a second name on a call that deserves one.
...
- It catches the second fault — the one that was hiding behind the first — while you are still standing there and still getting paid for it.
...
On the job, not in your head, not in a text to dispatch. If it is not in ServiceTitan it did not happen (`sop.csm.tools`).
...
They are not paying for readings, they are paying for an answer, and readings are how you get one. Narrate it and it reads as thorough: *"I'm taking a few measurements so I can tell you exactly what failed instead of guessing."* Nobody has ever objected to that sentence.

**"I've been doing this twenty years, I know what it is by the sound."**
You are probably right. Take the reading anyway — it takes forty seconds and it converts your judgment into something that survives being questioned by a customer, a manufacturer, or a warranty adjuster who was not in the room. Experience tells you where to look. The number is what you get to keep.

**"It's obviously the capacitor."**
Then it is a forty-second measurement and you were right. If it is not, you just avoided replacing a good part and leaving the real fault in the system — with your name on the invoice.

**"I don't have time for all this on a stacked day."**
Then the note says what you did not get to and why. An explained gap is a record; a silent gap is a defect. And be honest about the arithmetic: the twenty minutes you save today is a two-hour unpaid callback in three weeks, and it lands on whoever is free — often you.

**"The office just wants data."**
Some of it, yes. Not the callback you did not have to drive to, not the $1,400 repair that got approved because you showed a number, and not the warranty dispute that ended the moment your readings came out. Those are yours.
