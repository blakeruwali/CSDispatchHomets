...
- **Documentation layout shipped.** `/csm` and `/field` share one book-style layout (`DocsLayout`) — contents sidebar, single document on a readable measure, on-page contents rail, prev/next paging, light/dark, and print styles. `ContentSectionCard` was removed; nothing imported it after the switch.
- **"Suggest an edit" shipped.** `content_suggestions` exists in Supabase (migrations `20260804023711`, `20260804023736`) and the button lives in `DocsLayout`, so every surface gets it. Submissions are immutable; only review fields can change, and a reviewer must stamp themselves.
